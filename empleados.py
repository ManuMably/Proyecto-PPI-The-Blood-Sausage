...
from PyQt5.QtWidgets import QMainWindow, QDesktopWidget, QWidget, QVBoxLayout, QLabel, QApplication, QHBoxLayout, \
    QStyle, QPushButton, QLineEdit, QFormLayout, QScrollArea, QTableWidget, QTableWidgetItem, QMessageBox
...
class Empleados(QMainWindow):
    ...
    def __init__(self, menuPrincipal=None):
        ...
        # En este punto tenemos la lista usuarios con todos los Empleados
        ...
        for u in self.usuarios:
            self.tabla.setItem(self.contador, 0, QTableWidgetItem(u.nombreEmpleado))
            # Hacemos que el nombre no se pueda editar:
            self.tabla.item(self.contador, 0).setFlags(Qt.ItemIsEnabled)
            self.tabla.setItem(self.contador, 1, QTableWidgetItem(u.contrasena))
            self.tabla.setItem(self.contador, 2, QTableWidgetItem(u.cargo))
            self.contador += 1
        ...
        if boton == QMessageBox.StandardButton.Yes:
            ...
            if (
                ...
                for u in usuarios:
                    ...
                    if (
                            u.nombreEmpleado == self.tabla.item(filaActual, 0).text() and
                            u.contrasena == self.tabla.item(filaActual, 1).text() and
                            u.cargo == self.tabla.item(filaActual, 2).text()
                    ):
            ...
            if not existeRegistro:
                ...
                for u in usuarios:
                    ...
                    if (u.nombreEmpleado == self.tabla.item(filaActual, 0).text()):
                        # Indicamos que encontramos el documento
                        existeNombreCompleto = True

                        # Volvemos a actualizar todos los datos del usuario:
                        u.nombreEmpleado = self.tabla.item(filaActual, 0).text()
                        u.contrasena = self.tabla.item(filaActual, 1).text()
                        u.cargo = self.tabla.item(filaActual, 2).text()

                        # abrimos el archivo en modo escritura escribiendo datos en binario.
                        self.file = open('archivos_planos/empleadosRegistrados.txt', 'wb')

                        # recorremos la lista de usuarios
                        # para guardar usuario por usuario en el archivo
                        for u in usuarios:
                            self.file.write(bytes(u.nombreEmpleado + ";"
                                                  + u.contrasena + ";"
                                                  + u.cargo, encoding='UTF-8'))
                ...
                if not existeNombreCompleto:
                    ...
                    self.file = open('archivos_planos/empleadosRegistrados.txt', 'ab')
